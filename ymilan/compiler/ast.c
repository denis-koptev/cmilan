...
void dump_ast_proc(FILE* stream, int nspaces, ast_node* ast)
{
    ...
    case Node_Break:
        fprintf(stream, "%sBREAK:\n", spaces);
        dump_ast_proc(stream, nspaces + FORMAT_INDENT, ast->sub[0]);
        break;

    case Node_Continue:
        fprintf(stream, "%sCONTINUE:\n", spaces);
        dump_ast_proc(stream, nspaces + FORMAT_INDENT, ast->sub[0]);
        break;

    case Node_Repeat:
        fprintf(stream, "%sREPEAT:\n", spaces);
        dump_ast_proc(stream, nspaces + FORMAT_INDENT, ast->data.ast_value);
        dump_ast_proc(stream, nspaces + FORMAT_INDENT, ast->sub[0]);
        break;

    default:
        fprintf(stream, "%s<-- UNKNOWN NODE TYPE %d -->\n", spaces, ast->type);
        break;
    }
...
ast_node* new_repeat(ast_node* cond, ast_node* arg)
{
    ast_node* node;
